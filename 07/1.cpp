...
int getIntCodeOutput(std::vector<int> input, int userInput, int phaseSetting)
{
    int i = 0;
...
for(int i=0; i<permutations.size(); i++){
    ...
    while(permVal.size() > 0) {
        std::cout << "Perm val in loop" << permVal << std::endl;
        int phase = permVal[0] - '0';
        std::cout << "Phase " << phase << std::endl;
        std::cout << "Input " << input << std::endl;
        std::vector<int> ampProgram = parsedCsv;
        //std::cout << "Phase " << phase << std::endl;
        input = getIntCodeOutput(ampProgram, input, phase);
        std::cout << "Finished Amp" << std::endl;
        permVal = permVal.substr(1);
    }
    if(input > maxThrust) {
        maxThrust = input;
        ptr = i;
